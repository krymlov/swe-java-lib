...
public class TCPlanetPlanet extends TransitCalculator implements java.io.Serializable {
    ...
    //private double minVal = 0., maxVal = 0.;  // Thinking about it...
     ...
     * rather slow calculation and not finally tested.
     * <p>
     * 
     ...
     *            <br>
     *            For <i>right ascension</i> use <code>SEFLG_TRANSIT_LONGITUDE | SEFLG_EQUATORIAL</code>,
     ...
     */
    public TCPlanetPlanet(ISwissEph sw, int pl1, int pl2, int flags, double offset) {
     ...
     *            SweConst.SEFLG_PARTILE_TRANSIT_END to calculate partile aspects).<br>
     *            <br>
     *            Also flags modifying the basic planet calculations, these are
     *            SweConst.SEFLG_TOPOCTR, SweConst.SEFLG_EQUATORIAL, SweConst.SEFLG_HELCTR,
     ...
     * @see swisseph.SweConst#SEFLG_JPLEPH
     */
    public TCPlanetPlanet(ISwissEph sw, int pl1, int pl2, int flags, double offset, int precalcCount,
        ...
        this.tflags = flags;
        int vFlags = SweConst.SEFLG_EPHMASK | SweConst.SEFLG_TOPOCTR | SweConst.SEFLG_EQUATORIAL
            | SweConst.SEFLG_HELCTR | SweConst.SEFLG_NOABERR | SweConst.SEFLG_NOGDEFL | SweConst.SEFLG_SIDEREAL
            | SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_PARTILE_TRANSIT | SweConst.SEFLG_PARTILE_TRANSIT_START
            | SweConst.SEFLG_PARTILE_TRANSIT_END | SweConst.SEFLG_YOGA_TRANSIT
            | SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE
        ...
        if ( type != SweConst.SEFLG_TRANSIT_LONGITUDE && type != SweConst.SEFLG_TRANSIT_LATITUDE
            ...
            throw new IllegalArgumentException("Invalid flag combination '" + flags
                ...
                + "), SEFLG_TRANSIT_LATITUDE (" + SweConst.SEFLG_TRANSIT_LATITUDE
                + "), SEFLG_TRANSIT_DISTANCE (" + SweConst.SEFLG_TRANSIT_DISTANCE + ").");
        }
        if ( (flags & SweConst.SEFLG_HELCTR) != 0
            && (pl1 == SweConst.SE_MEAN_APOG || pl1 == SweConst.SE_OSCU_APOG || pl1 == SweConst.SE_MEAN_NODE
                || pl1 == SweConst.SE_TRUE_NODE) ) {
            throw new IllegalArgumentException("Unsupported planet number " + pl1 + " ("
                + sw.swe_get_planet_name(pl1) + ") for heliocentric " + "calculations");
        }
        if ( (flags & SweConst.SEFLG_HELCTR) != 0
            && (pl2 == SweConst.SE_MEAN_APOG || pl2 == SweConst.SE_OSCU_APOG || pl2 == SweConst.SE_MEAN_NODE
                || pl2 == SweConst.SE_TRUE_NODE) ) {
            throw new IllegalArgumentException("Unsupported planet number " + pl2 + " ("
                + sw.swe_get_planet_name(pl2) + ") for heliocentric " + "calculations");
        }
        if ( pl1 == pl2 ) {
        ...
        flags &= ~(SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE
            | SweConst.SEFLG_TRANSIT_DISTANCE | SweConst.SEFLG_PARTILE_TRANSIT
            | SweConst.SEFLG_PARTILE_TRANSIT_START | SweConst.SEFLG_PARTILE_TRANSIT_END
            | SweConst.SEFLG_YOGA_TRANSIT | SweConst.SEFLG_TRANSIT_SPEED);
        this.flags = flags;
        ...
        if ( calcPartile || calcNonPartile ) {
            ...
            if ( !calcPartileBoth ) {
                if ( isPartile && calcPartile ) {
                    ...
                    jdET = getTransitET(tc, jdET, back);
                    isPartile = !isPartile;
    ...
    public boolean hasPartileAspect(double jdET, int p1, int p2, int flgs, double offset) {
        ...
        flgs &= ~(SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE
            | SweConst.SEFLG_TRANSIT_DISTANCE | SweConst.SEFLG_PARTILE_TRANSIT
            | SweConst.SEFLG_PARTILE_TRANSIT_START | SweConst.SEFLG_PARTILE_TRANSIT_END
            | SweConst.SEFLG_YOGA_TRANSIT | SweConst.SEFLG_TRANSIT_SPEED);
    ...
    protected double getDegreePrecision(double jd) {
        ...
        switch ( idx ) {
            ...
            case 5: // Speed in distance
                ...
                double dp1, dp2;
                if ( pl1 >= Extensions.maxBaryDist.length ) {
                    dp1 = 0.05; // Rather random value???
                } else {
                    dp1 = Extensions.maxBaryDist[pl1];
                }
                if ( pl2 >= Extensions.maxBaryDist.length ) {
                    dp2 = 0.05; // Rather random value???
                } else {
                    dp2 = Extensions.maxBaryDist[pl2];
                }
    ...
    private double getSpeed(boolean min, int planet, boolean speed) {
        ...
        try {
            ...
            if ( topo ) {
                ...
                //        }
                if ( sw.swe_get_geo_alt() > 50000. || sw.swe_get_geo_alt() < -12000000 ) {
                    return 1. / 0.;
                }
                if ( speed ) {
                    if ( rect ) {
