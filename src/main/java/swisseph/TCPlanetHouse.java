...
public class TCPlanetHouse extends TransitCalculator implements java.io.Serializable {
     ...
     * sw.swe_set_sid_mode(...) prior to using this TransitCalculator.
     * <p>
     ...
     *            these extreme speeds will be used on calculation.<br>
     *            Other objects
     ...
     * @param planetFlags The flags for calculation of the planet.
     *            <p>
     ...
     */
    public TCPlanetHouse(ISwissEph sw, int planet, int planetFlags, int houseObject, int houseSystem,
        int houseFlags, double houseGeolon, double houseGeolat, double offset) {
        this(sw, planet, planetFlags, houseObject, houseSystem, houseFlags, houseGeolon, houseGeolat, offset,
            200, 1.4);
    }
     ...
     */
    public TCPlanetHouse(ISwissEph sw, int planet, int planetFlags, int houseObject, int houseSystem,
        int houseFlags, double houseGeolon, double houseGeolat, double offset, int precalcCount,
        ...
        // Check planet flags:
        int vPlFlags = SweConst.SEFLG_EPHMASK | SweConst.SEFLG_TOPOCTR | SweConst.SEFLG_EQUATORIAL
            | SweConst.SEFLG_HELCTR | SweConst.SEFLG_NOABERR | SweConst.SEFLG_NOGDEFL | SweConst.SEFLG_SIDEREAL
            | SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_TRANSIT_LONGITUDE;
        ...
        if ( this.transitFlags != SweConst.SEFLG_TRANSIT_LONGITUDE ) {
            throw new IllegalArgumentException("Invalid flag combination (" + planetFlags + " or " + houseFlags
                + ")" + ": only SEFLG_TRANSIT_LONGITUDE (" + SweConst.SEFLG_TRANSIT_LONGITUDE
                + ") is allowed as transit method.");
        }
        if ( (planetFlags & SweConst.SEFLG_HELCTR) != 0
            && (planet == SweConst.SE_MEAN_APOG || planet == SweConst.SE_OSCU_APOG
        ...
        int object = houseObject & (SweConst.SE_HOUSE1 | SweConst.SE_HOUSE2 | SweConst.SE_HOUSE3
            | SweConst.SE_HOUSE4 | SweConst.SE_HOUSE5 | SweConst.SE_HOUSE6 | SweConst.SE_HOUSE7
            | SweConst.SE_HOUSE8 | SweConst.SE_HOUSE9 | SweConst.SE_HOUSE10 | SweConst.SE_HOUSE11
            | SweConst.SE_HOUSE12 | SweConst.SE_ASC | SweConst.SE_MC | SweConst.SE_ARMC | SweConst.SE_VERTEX
        ...
        if ( houseSystem != SweConst.SE_HSYS_PLACIDUS && houseSystem != SweConst.SE_HSYS_KOCH
            && houseSystem != SweConst.SE_HSYS_PORPHYRIUS && houseSystem != SweConst.SE_HSYS_REGIOMONTANUS
            && houseSystem != SweConst.SE_HSYS_CAMPANUS && houseSystem != SweConst.SE_HSYS_EQUAL
            && houseSystem != SweConst.SE_HSYS_VEHLOW && houseSystem != SweConst.SE_HSYS_MERIDIAN
            && houseSystem != SweConst.SE_HSYS_HORIZONTAL && houseSystem != SweConst.SE_HSYS_POLICH_PAGE
            && houseSystem != SweConst.SE_HSYS_ALCABITIUS &&
            //        houseSystem != SweConst.SE_HSYS_GAUQUELIN_SECTORS &&
            houseSystem != SweConst.SE_HSYS_MORINUS && houseSystem != SweConst.SE_HSYS_KRUSINSKI
            && houseSystem != SweConst.SE_HSYS_WHOLE_SIGN ) {
            throw new IllegalArgumentException("Unsupported house system '" + houseSystem + "'.");
        }
        ...
        if ( Double.isInfinite(maxSpeed1) || Double.isInfinite(minSpeed1) ) {
            throw new IllegalArgumentException(
                ((planetFlags & SweConst.SEFLG_TOPOCTR) != 0 ? "Topo" : ((planetFlags
                    & SweConst.SEFLG_HELCTR) != 0 ? "Helio" : "Geo"))
                    + "centric transit calculations with planet number " + planet + " ("
                    + sw.swe_get_planet_name(planet) + ") not possible: extreme speeds" + " of the planet "
        ...
        if ( Double.isInfinite(maxSpeed2) || Double.isInfinite(minSpeed2) ) {
            throw new IllegalArgumentException(
                ((houseFlags & SweConst.SEFLG_TOPOCTR) != 0 ? "Topo" : ((houseFlags
                    & SweConst.SEFLG_HELCTR) != 0 ? "Helio" : "Geo"))
     ...
     * precision had been reached. The default for the precision factor is 1.
     * <p>
     * 
    ...
    private double getSpeed(boolean min, int planet) {
        ...
        try {
            ...
            if ( topo ) {
                ...
                //        }
                final double geoalt = sw.swe_get_geo_alt();
                if ( geoalt > 50000. || geoalt < -12000000 ) {
                    return 1. / 0.;
                }
                if ( rect ) {
    ...
    double[] getTestspeed(int planet, int idx, int precalcCount, double precalcSafetyfactor) {
        ...
        final double[] timerange = new double[] { SwephData.MOSHPLEPH_START, SwephData.MOSHPLEPH_END };
        if ( planet > SweConst.SE_AST_OFFSET ) {
            throw new IllegalArgumentException("Planet > SE_AST_OFFSET is not supported");
            /*
            // get filename:
            ...
            try {
            fptr = sw.swi_fopen(SwephData.SEI_FILE_ANY_AST, fn, sw.getEphePath(), serr);
    ...
    public String toString() {
        return "[Planet/house/house system:" + planet + "/" + houseObject + "/" + houseSystem + "];Offset:"
            + getOffset();
    }
}
