...
public class SweDate implements Serializable {
   ...
   * Useful for conversions from or to a Date object.
   */
  public static final double JD0 = 2440587.5; /* 1970 January 1.0 */
  
  /**
   * The Julian day number for the start of the Gregorian calendar system (October 15, 1582)
   */
  public static final double JD_GC0 = 2299160.5;

  public static final IDate DT_GC0 = swe_revjul(JD_GC0, SE_GREG_CAL);
  ...
  private static boolean init_leapseconds_done=true;
  
  // JD for the start of the Gregorian calendar system (October 15, 1582):
  private double jdCO = JD_GC0;
...
//  int getYear(double jd /*, boolean calType ?*/) { swe_revjul(jd,calType); }
  ...
  * Delta T is calculated with a tidal acceleration of SE_TIDAL_DEFAULT.
  ...
  */
  public void setCalendarType(boolean newCalType, boolean keepDate) {
    if (this.calType != newCalType) {
      ...
      deltatIsValid=false;
      if (keepDate) {
        this.julDay=swe_julday(this.year, this.month, this.day,
                           this.hour, this.calType);
      } else {
        IDate dt=swe_revjul(this.julDay,newCalType);
        this.year=dt.year;
        this.month=dt.month;
        this.day=dt.day;
        this.hour=dt.hour;
  ...
  */
  public void updateCalendarType() {
    this.calType=(this.jdCO<=this.julDay?SE_GREG_CAL:SE_JUL_CAL);;
  }
  ...
  */
  public boolean setDate(int newYear, int newMonth, int newDay, double newHour,
    ...
    if (check) {
      ...
      IDate dt=swe_revjul(this.julDay,this.calType);  // -> erzeugt neues Datum
      this.year=dt.year;
      this.month=dt.month;
      this.day=dt.day;
      this.hour=dt.hour;
  ...
  */
  public void setGregorianChange(int year, int month, int day) {
    ...
    this.julDay = swe_julday(this.year, this.month, this.day, this.hour,
                         this.calType);
  }
  ...
  */
  public void setGregorianChange(double newJDCO) {
    this.jdCO = newJDCO;
    this.calType = (this.julDay>=this.jdCO?SE_GREG_CAL:SE_JUL_CAL);
    IDate dt = swe_revjul(this.julDay,this.calType);
    this.year = dt.year;
    this.month = dt.month;
    this.day = dt.day;
    this.hour = dt.hour;
...
// tid_acc.set(t_acc);
  ...
  * @see swisseph.SweConst#SE_TIDAL_DEFAULT
  */
  static synchronized void setGlobalTidalAcc(double tjd_ut, int iflag, int denum) {
    final SwissEph sw = getSwissEph();
...
//      default: tid_acc.set(SweConst.SE_TIDAL_DEFAULT); break;
  ...
  */
  protected static synchronized void setSwissEphObject(SwissEph swiss) {
      if ( null == SW ) SW = swiss;
  }
 ...
 ****************************************************************/
  public static double swe_julday(int year, int month, int day,
  ...
  //////////////////////////////////////////////////////////////////////
  public static final IDate swe_revjul(double jd, boolean calType) {
    final IDate dt = new IDate();
  ...
  private static final int TABEND=2028;
  ...
  private static final double dt[]=new double[] {
  // Table of Astronomical Almanac / IERS values, copied from swephlib.c of
  // Swiss Ephemeris 2.10.03. It used to stop at 2019 with four extrapolated
  // entries; it now carries measured values through 2023 and the same
  // extrapolation to 2028 that the native library uses.
  /* 1620.0 - 1659.0 */
...
/*#define DELTAT_ESPENAK_MEEUS_2006 TRUE*/
  ...
  public static double calc_deltaT(double tjd) {
     ...
     */
    /* Model for epochs before 1955, the default in Swiss Ephemeris since 2.06:
     * Stephenson/Morrison/Hohenkerk 2016. We switch over to Astronomical Almanac K8-K9
     * and IERS at 1 Jan 1955, with a linear term over the 1000 days before that date to
     * keep the curve continuous. */
    if (deltat_model == SweConst.SEMOD_DELTAT_STEPHENSON_ETC_2016 && tjd < 2435108.5) {
      double d = deltat_stephenson_etc_2016(tjd);
      if (tjd >= 2434108.5) {
  ...
  private static double deltat_aa(double tjd) {
     ...
     */
    int deltat_model = getSwissEph().swed.astro_models[SweConst.SE_MODEL_DELTAT];
    if (deltat_model == 0) deltat_model = SweConst.SEMOD_DELTAT_DEFAULT;
    if (deltat_model == SweConst.SEMOD_DELTAT_STEPHENSON_ETC_2016) {
      /* 3rd degree polynomial based on data given by
       * Stephenson/Morrison/Hohenkerk 2016, http://astro.ukho.gov.uk/nao/lvm/
       * The pure Java port only had the Stephenson 1997 parabola below, which is why it
       * disagreed with the native engine by 122 seconds at the year 2099. */
      B = (Y - 2000);
      if (Y < 2500) {
        ...
        /* a parabola after 2500 */
        B = 0.01 * (Y - 2000);
        ans = B * B * 32.5 + 42.5;
        B2 = 0.01 * (tabend - 2000);
        ans2 = B2 * B2 * 32.5 + 42.5;
      }
    } else {
      B = 0.01 * (Y - 1820);
      ans = -20 + 31 * B * B;
    ...
    /* slow transition from tabulated values to the formula: */
    if (Y <= tabend+100) {
      ans3 = dt[tabsiz-1];
      dd = (ans2 - ans3);
  ...
  private static final int NDTCF16 = 54;

  /**
   * Spline coefficients of Stephenson/Morrison/Hohenkerk 2016, taken verbatim from
   * swephlib.c (dtcf16). Columns: tjd start, tjd end, then the four polynomial
   * coefficients in t = (tjd - start) / (end - start).
   */
  private static final double dtcf16[][] = new double[][] {
  /*00*/ {1458085.5, 1867156.5, 20550.593,-21268.478, 11863.418, -4541.129},   // -720 .. 400
  /*01*/ {1867156.5, 2086302.5,  6604.404, -5981.266,  -505.093,  1349.609},   // 400 .. 1000
  /*02*/ {2086302.5, 2268923.5,  1467.654, -2452.187,  2460.927, -1183.759},   // 1000 .. 1500
  /*03*/ {2268923.5, 2305447.5,   292.635,  -216.322,   -43.614,    56.681},   // 1500 .. 1600
  /*04*/ {2305447.5, 2323710.5,    89.380,   -66.754,    31.607,   -10.497},   // 1600 .. 1650
  /*05*/ {2323710.5, 2349276.5,    43.736,   -49.043,     0.227,    15.811},   // 1650 .. 1720
  /*06*/ {2349276.5, 2378496.5,    10.730,    -1.321,    62.250,   -52.946},   // 1720 .. 1800
  /*07*/ {2378496.5, 2382148.5,    18.714,    -4.457,    -1.509,     2.507},   // 1800 .. 1810
  /*08*/ {2382148.5, 2385800.5,    15.255,     0.046,     6.012,    -4.634},   // 1810 .. 1820
  /*09*/ {2385800.5, 2389453.5,    16.679,    -1.831,    -7.889,     3.799},   // 1820 .. 1830
  ...
  /**
   * Delta t according to Stephenson/Morrison/Hohenkerk 2016, the model the native library
   * uses by default for every date before 1 Jan 1955. The pure Java port had no equivalent
   * and fell back to Espenak &amp; Meeus 2006, which is why it disagreed with the native
   * engine by 94 seconds at the year 1000 and 97 seconds at 1500.
   * <p>
   * Ported from <code>deltat_stephenson_etc_2016()</code> in swephlib.c.
   */
  private static double deltat_stephenson_etc_2016(double tjd) {
    double t, dt, Ygreg;
    int irec = -1;
    Ygreg = 2000.0 + (tjd - SwephData.J2000) / 365.2425;
    // after the year -720 get the value from the spline curve
    for (int i = 0; i < NDTCF16; i++) {
      if (tjd < dtcf16[i][0]) break;
      if (tjd < dtcf16[i][1]) { irec = i; break; }
    }
    if (irec >= 0) {
      t = (tjd - dtcf16[irec][0]) / (dtcf16[irec][1] - dtcf16[irec][0]);
      dt = dtcf16[irec][2] + dtcf16[irec][3] * t + dtcf16[irec][4] * t * t
          + dtcf16[irec][5] * t * t * t;
    // for earlier epochs, use the long term parabola
    } else if (Ygreg < -720) {
      t = (Ygreg - 1825) / 100.0;
      dt = -320 + 32.5 * t * t;
      dt -= 179.7337208;   // to make curve continuous at -720
    // for later epochs, use the long term parabola
    } else {
      t = (Ygreg - 1825) / 100.0;
      dt = -320 + 32.5 * t * t;
      dt -= 1.517032;      // to make curve continuous at 2016
    }
    // The parameters of the above formulae are for a tidal acceleration of
    // SE_TIDAL_STEPHENSON_2016, and the adjustment is applied for all epochs, not only
    // before 1955.
    dt = adjust_for_tidacc(dt, Ygreg, SweConst.SE_TIDAL_STEPHENSON_2016, true);
    dt /= 86400.0;
    return dt;
  }
  ...
  private static double deltat_aa_label_done(double ans, double Y) {
    ans = adjust_for_tidacc(ans, Y);
    return ans / 86400.0;
  }
  ...
  private static int init_dt() {
    // sedeltat.txt.inactive, so, skip it
    if ( !INIT_DT_DONE ) {
        INIT_DT_DONE = true;
        return TABSIZ;
    }
  ...
  private static double adjust_for_tidacc(double ans, double Y) {
    return adjust_for_tidacc(ans, Y, SweConst.SE_TIDAL_26, false);
  }

  /**
   * Generalised form, matching <code>adjust_for_tidacc()</code> in swephlib.c: the tidal
   * acceleration the tabulated values were derived with is a parameter, and models such as
   * Stephenson/Morrison/Hohenkerk 2016 need the correction applied after 1955 as well.
   */
  private static double adjust_for_tidacc(double ans, double Y, double tid_acc0,
      boolean adjust_after_1955) {
    double B;
  ...
  */
  private void setFields(int year, int month, int day, double hour) {
    // Get year, month, day of jdCO and compare to given date to find out about the calendar system:
    IDate dt = DT_GC0;
    if ( jdCO != JD_GC0 ) dt = swe_revjul(jdCO, SE_GREG_CAL);
  ...
  * object.
  */
  protected void setFields(int year, int month, int day, double hour, boolean calType) {
    this.year=year;
    this.month=month;
    this.day=day;
    this.hour=hour;
    this.calType=calType;
    this.julDay=swe_julday(year, month, day, hour, calType);
  }
  ...
  */
  public static SDate getUTCFromLocalTime(int iyear, int imonth, int iday,
  ...
  */
   ...
   */
  private static int init_leapsec() {
    FilePtr fp = null;
    int ndat, ndat_last;
    int tabsiz = 0;
    int i;
    ...
    /* find table size */
    tabsiz = NLEAP_SECONDS;
    for (i = NLEAP_SECONDS; i < NLEAP_SECONDS_SPACE; i++) {
      if (leap_seconds[i] == 0)
        break;
      else
        tabsiz++;
  ...
  */
  public static boolean isValidUTCDate(int iyear, int imonth, int iday, int ihour, int imin, double dsec, boolean gregflag) {
  ...
  */
  public static String getInvalidUTCDateError(int iyear, int imonth, int iday, int ihour, int imin, double dsec, boolean gregflag) {
...
//   swe_revjul(tjd_ut1, gregflag, &iyear2, &imonth2, &iday2, &d);
  ...
  */
   ...
   *   for a long time.
  */
  public static double[] getJDfromUTC(int iyear,
      int imonth,
      int iday,
      int ihour,
      int imin,
...
//      swe_revjul(tjd_ut1, gregflag, &iyear, &imonth, &iday, &d);
  ...
  */
   ...
   */
  public static SDate getUTCfromJDET(double tjd_et, boolean gregflag) {
    int i;
    int second_60 = 0;
...
//     swe_revjul(tjd, gregflag, iyear, imonth, iday, &d);
  ...
  */
   ...
   *   updated for a long time.
   */
  public static SDate getUTCfromJDUT1(double tjd_ut, boolean gregflag) {
