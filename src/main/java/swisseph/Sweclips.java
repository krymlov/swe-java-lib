...
public class Sweclips
  ...
  */
  ...
  void main_start(String[] argv) {
    String saves;
    String s1, s2;
    StringBuilder serr=new StringBuilder(swed.AS_MAXCH),
                 serr_save=new StringBuilder(swed.AS_MAXCH),
                 serr_warn=new StringBuilder(swed.AS_MAXCH);
    String sout;
    String sp, sp2;
    String fmt = "PLBRS";
    int i, ii;
...
// JAVA: Moeglicherweise inkompatibel mit der C Variante
        ...
        if (argv[i].length() == 3)
          ...
          starname=new StringBuilder(argv[i].substring(2));
...
//        swe_revjul(t_ut, gregflag, &jyear, &jmon, &jday, &jut);
      ...
      StringBuilder sbstar=new StringBuilder(starname.toString());
      if ((smod & SMOD_RISE)!=0) {
        dtretx.val=tret[0];
...
//  end_main:
  ...
  int do_calc(double tjd, int ipl, int iflag, double[] x, StringBuilder serr) {
    return sw.swe_calc(tjd, ipl, iflag, x, serr);
  }

  void do_printf(StringBuilder info) {
