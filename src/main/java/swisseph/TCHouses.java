...
//  * @see swisseph.SweConst#SE_HSYS_GAUQUELIN_SECTORS
  ...
  */
  public TCHouses(ISwissEph sw, int houseObject, int hsys, double geolon, double geolat, int flags, double offset) {
      super(sw);
...
//        hsys != SweConst.SE_HSYS_GAUQUELIN_SECTORS &&
  ...
  */
  ...
  protected double calc(double jd) {
    double[] cusps = new double[(this.hsys == SweConst.SE_HSYS_GAUQUELIN_SECTORS ? 38 : 14)];
    double[] ascmc = new double[10];


    sw.swe_set_topo(geolon, geolat, 0);
    int ret = sw.swe_houses_ex(jd, flags, geolat, geolon, this.hsys, cusps, ascmc);
